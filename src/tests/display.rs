...
fn shown(formula: &str) -> String {
    parsed(formula).to_string()
}

#[test]
fn display_parenthesises_only_where_precedence_or_associativity_requires() {
...
#[test]
fn display_keeps_long_chains_flat() {
    let chain = vec!["#1"; crate::MAX_DEPTH].join(" - ");
    assert_eq!(shown(&chain), chain);
    assert_eq!(parsed(&shown(&chain)), parsed(&chain));
}

#[test]
fn display_does_not_round_trip_other_keys_or_negative_constants() {
    let other_key = Formula::<f32, String>::Component("v1".to_string());
    assert_eq!(other_key.to_string(), "#v1");
    assert!(parse::<f32>(&other_key.to_string()).is_err());

    let negative = Formula::<f32>::Constant(Some(-2.0));
    assert_eq!(negative.to_string(), "-2");
    assert_eq!(
        parsed(&negative.to_string()),
        Formula::Neg(Box::new(Formula::Constant(Some(2.0))))
    );
}
...
fn display_round_trips_through_parse() {
    ...
    for formula in formulas {
        let expr = parsed(formula);
        let reparsed = parsed(&expr.to_string());
