...
- `FormulaError` is a `#[non_exhaustive]` enum with one variant per failure (`Syntax`, `InvalidNumber`, `NumberOutOfRange`, `InvalidComponentId`, `NestedTooDeep`, `TooDeep`, `Arity` and `Internal`) instead of a tuple struct around a message, so a `match` on it needs a `_` arm. `Display` still renders the same messages.
