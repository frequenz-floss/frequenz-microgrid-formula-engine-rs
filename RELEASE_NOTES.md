...
- The `traits` module and its `NumberLike` trait are removed. Numeric types are bounded by `Float` instead, re-exported from `num_traits`, which `f32` and `f64` implement, so a bound written `T: NumberLike<T> + PartialOrd` becomes `T: Float`. `Float` asks for the whole floating-point API, so a custom numeric type that implemented `NumberLike` has to implement `num_traits::Float` instead, which means depending on `num-traits` 0.2 directly.
...
- A formula never evaluates to a NaN or infinite value. A reading, a hand-built constant or an intermediate result that is not finite is `None`. Division by a zero divisor is one such result: `1 / 0` used to yield infinity and `0 / 0` NaN. `COALESCE` and `AVG` move past such a value like any other missing value, and `MIN` and `MAX` yield `None`. Such values used to pass through, and the result of `MIN` and `MAX` with a NaN argument depended on the argument order.
